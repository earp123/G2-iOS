...
struct HistoryView: View {
        ...
        Group {
            if !history.hasData {
                ...
                List {
                    ...
                    Section {
                        ForEach(history.listRecords) { record in
    ...
    /// True in-range total, with a note when the list is capped for smoothness.
    private var listHeader: String {
        if history.totalInRange > HistoryStore.listLimit {
            return "\(history.totalInRange) records · showing latest \(HistoryStore.listLimit)"
        }
        return "\(history.totalInRange) records · newest first"
    }
    ...
    private var syncStatusRow: some View {
        if history.syncState == .syncing {
            // Live transfer progress — u24 index/total from the stream (§ protocol).
            HStack(spacing: 10) {
                if let fraction = history.syncProgress {
        ...
        } else if case let .result(result) = history.syncState {
            HStack(spacing: 8) {
    ...
    private var emptyState: some View {
        ContentUnavailableView {
            ...
            if history.syncState == .syncing {
                // First sync can be a long full dump — show live transfer progress.
                VStack(spacing: 8) {
                    if let fraction = history.syncProgress {
                        ProgressView(value: fraction).tint(Theme.accent).frame(maxWidth: 220)
                        Text("Syncing… \(Int(fraction * 100))%")
                            .font(.caption.monospacedDigit())
                            .foregroundStyle(Theme.textSecondary)
                    } else {
                        ProgressView().tint(Theme.accent)
                        Text("Syncing…").font(.caption).foregroundStyle(Theme.textSecondary)
                    }
                }
            } else {
                Button {
